...
public class HashMap<K,V> extends AbstractMap<K,V>
    ...
    static final int hash(Object key) {
        int h;
        // key 为空 则返回0 所以HashMap中可以存储null
        // key.hashCode() ^ (key.hashCode() >>> 16) 表示 hashCode 高16位和低16位进行异或运算 不用管太多 核心设计是为了减少hash碰撞
     ...
     * @serial
     */
    // 负载因子 默认是0.75 也就是 table 数组的容量超过75%的时候会进行扩容
    // 用于计算什么时候table 数组的 需要扩容
